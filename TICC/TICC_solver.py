...
from TICC.src.TICC_helper import *
from TICC.src.admm_solver import ADMMSolver
from TICC.f1_score_compute import f1
...
class TICC:
    ...
    def fit(self, input_file):
        ...
        for iters in tnrange(self.maxIters, desc=("Training Iteration - (max " + str(self.maxIters) + ")")):
    ...
    def compute_f_score(self, matching_EM, matching_GMM, matching_Kmeans, train_confusion_matrix_EM,
        ...
        # F1 Score calculator
        f1_EM_tr = f1.computeF1_macro(train_confusion_matrix_EM,matching_EM,self.number_of_clusters)
        f1_GMM_tr = f1.computeF1_macro(train_confusion_matrix_GMM,matching_GMM,self.number_of_clusters)
        f1_kmeans_tr = f1.computeF1_macro(train_confusion_matrix_kmeans,matching_Kmeans,self.number_of_clusters)
        print("\n\n")
        print("TRAINING F1 scores:")
        print("EM (expectation maximization):", f1_EM_tr.round(2))
        print("GMM (Gaussian mixture model):", f1_GMM_tr.round(2))
        print("k-Means:", f1_kmeans_tr.round(2))
        print("\n")
    ...
    def optimize_clusters(self, computed_covariance, len_train_clusters, log_det_values, optRes, train_cluster_inverse):
        for cluster in tnrange(self.number_of_clusters, desc=("Clusteroptimierung")):
